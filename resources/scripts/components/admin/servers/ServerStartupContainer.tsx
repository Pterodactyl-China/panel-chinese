...
function ServerStartupLineContainer({ egg, server }: { egg?: Egg; server: Server }) {
    ...
    return (
        <AdminBox title={'启动命令'} className="relative w-full">
            ...
            <div className="mb-6">
                <Field
                    ...
                    type={'text'}
                    description={
                        '在此处编辑服务器的启动命令。以下变量默认可用：{{SERVER_MEMORY}}、{{SERVER_IP}} 和 {{SERVER_PORT}}。'
                    }
...
export function ServerServiceContainer({
    ...
    return (
        <AdminBox title={'服务配置'} isLoading={isSubmitting} className="w-full">
            <div className="mb-6">
            ...
            <div className="bg-neutral-800 border border-neutral-900 shadow-inner p-4 rounded">
                <FormikSwitch name={'skipScripts'} label={'跳过预设安装脚本'} description={'Soon™'} />
            </div>
        </AdminBox>
    );
}
...
export function ServerImageContainer() {
    ...
    return (
        <AdminBox title={'镜像配置'} className="relative w-full">
            <SpinnerOverlay visible={isSubmitting} />
            ...
            <div className="md:w-full md:flex md:flex-col">
                <div>
                    ...
                    <Field id={'image'} name={'image'} label={'Docker 镜像'} type={'text'} />
                </div>
            </div>
        </AdminBox>
    );
